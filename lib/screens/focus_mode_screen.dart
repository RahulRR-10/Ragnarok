...
class _FocusModeScreenState extends State<FocusModeScreen>
  ...
  // Using static variables to persist state across screen navigation
  static final audioPlayer = AudioPlayer();
  static String _selectedSound = 'none';
  static double _volume = 0.5;
  static bool _isLoadingAudio = false;
  static Timer? _audioLoadingTimer;
  ...
  final List<Map<String, String>> _quotes = [
    ...
    {
      'quote':
          '"The successful warrior is also just the average man, except with laser-like focus."',
      'author': '- Bruce Lee',
    },
    {
      'quote':
    ...
    {
      'quote':
          '"Concentrate your thoughts upon the work in hand. The sun\'s rays do not burn until brought to focus."',
  ...
  Future<void> _initializeAudio() async {
    try {
      // Only set loop mode if it hasn't been initialized yet
  ...
  void _resetAudioLoadingState() {
    _audioLoadingTimer?.cancel();
    setState(() {
  ...
  Future<void> _playSound(String soundName) async {
    ...
    try {
      ...
      _audioLoadingTimer = Timer(const Duration(seconds: 5), () {
        if (mounted && _isLoadingAudio) {
          debugPrint('Audio loading timeout for $soundName');
          _resetAudioLoadingState();
        }
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Failed to play $soundName sound. Please check if the file exists.'),
  ...
  void dispose() {
    _timer?.cancel();
    _quoteTimer.cancel();
    // Don't dispose the audio player since it's static now
    // Only cancel the timer if we're leaving the app completely
    if (!mounted) {
      _audioLoadingTimer?.cancel();
    }
